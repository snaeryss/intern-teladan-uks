...
return new class extends Migration
{
    public function up(): void
    {
        ...
        $foreignKeyExists = $this->foreignKeyExists('mcu', 'mcu_doctor_id_foreign');
        
        Schema::table('mcu', function (Blueprint $table) use ($foreignKeyExists) {
            // 1. Drop foreign key constraint lama (jika ada)
            if ($foreignKeyExists) {
                $table->dropForeign('mcu_doctor_id_foreign');
            }
            
            // 2. Drop kolom doctor_id lama (UUID)
            if (Schema::hasColumn('mcu', 'doctor_id')) {
